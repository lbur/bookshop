...
font: 12px/23px 'Microsoft YaHei';
overflow: hidden;
background-color:#f5350c; /*#f65b12cc;*/
color: #fff;
text-align: center;
border-radius: 3px;
line-height: -moz-block-height;
...
.tocalcu:hover{
text-decoration:none;
background-color: #e62e07;/*#fa5000;*/
color: #fff;
...
<body>
  <% User user = (User) request.getSession().getAttribute("users") ;
	   ...
	   ordernum="1111"+System.currentTimeMillis();
		//return 0;
    }
    ...
    <div class="carttop">
    <img src="./style/index/image/top2.png">
    </div>
    <span style="padding-left:40px">首页>用户订单</span>
	<form action="ShowOrderServlet" method="post">
		<table class="table table-bordered" align="center" border="0"
			...
			<tr>
				<td class="aa" align="center" colspan="7"><p class="text-primary">订单号:${dingdan}</p></td>
			</tr>
			<tr>
    			<td>书籍名</td>
    			<td>图书封面</td>
    ...
    <!--fmt:formatNumber 这个标签和c标签一样，也属于jstl标签库。表示按格式转换数字 。下面pattern="#.00"表示将数字保留2位小数进行转换 -->
    		...
    		<tr>
    			...
    			<td>${pprice}</td>
    			<td>${c.bcount}</td>
    			<td>${c.bcount * pprice}</td>
    			<!-- 下面的c set 标签 相当于 sum = 每行金额 + sum 这样一个求和语句-->
    			<c:set value="${sum+c.bcount * pprice}" var="sum" />
    		</tr>
    		</c:forEach>
    		<tr>
    			<fmt:formatNumber value="${sum}" pattern="#.00" var="formatSum"/>
    			<td colspan="7" align="right">应付款 ：${formatSum}元</td>
    		</tr>
    		<tr>
    		     <td colspan="7" align="right">收货人：${users.username}</td>
    		    
    		</tr>
    		<tr> <td colspan="7" align="right">收货地址：${users.addr}</td></tr>
    		<tr>
    		    <td colspan="3" align="right" ><a class="back" href="ShowCartListServlet">返回购物车</a></td>
    			<td colspan="4" align="right" ><a class="tocalcu" href="#">提交订单</a></td>
    		</tr>
